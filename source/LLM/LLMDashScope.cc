...
LLMDashScope::LLMDashScope(const Json &cfg) : LLMBase(cfg) {
  if (model_.find("vl") != std::string::npos ||
      model_.find("vision") != std::string::npos) {
    support_multimodal_ = true;
  }

  if (cfg.has("generate_cfg") && cfg["generate_cfg"].has("enable_thinking")) {
    enable_thinking_ =
        static_cast<bool>(cfg["generate_cfg"]["enable_thinking"]);
  }
}

void LLMDashScope::ChatImpl(const std::vector<Message> &messages,
...
void LLMDashScope::HttpCallback(WFHttpTask *task, LLMStreamCallback stream_cb,
                                 LLMCompleteCallback complete_cb,
    ...
    if (body_str.find("\"error\"") != std::string::npos) {
      ...
      if (err_start != std::string::npos) {
        ...
        if (err_json.is_valid() && err_json.has("error")) {
          std::string msg = "DashScope API error: ";
...
REGISTERLLM("dashscope", LLMDashScope)
