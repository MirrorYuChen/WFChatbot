...
LLMDeepSeek::LLMDeepSeek(const Json &cfg) : LLMBase(cfg) {}

void LLMDeepSeek::ChatImpl(const std::vector<Message> &messages,
...
void LLMDeepSeek::HttpCallback(WFHttpTask *task, LLMStreamCallback stream_cb,
                               LLMCompleteCallback complete_cb,
    ...
    if (body_str.find("\"error\"") != std::string::npos) {
      ...
      if (err_start != std::string::npos) {
        ...
        if (err_json.is_valid() && err_json.has("error")) {
          std::string msg = "DeepSeek API error: ";
...
REGISTERLLM("deepseek", LLMDeepSeek)
