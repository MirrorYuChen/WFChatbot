...
Json::Json(const std::vector<std::string> &val)
    : node_(json_value_create(JSON_VALUE_ARRAY)), parent_(nullptr),
      allocated_(true) {
...
Json Json::parse(const std::string &str) { return Json(str, true); }
...
void Json::push_back(const std::string &key,
...
void Json::normal_push_back(const std::string &key,
  ...
  if (find == nullptr) {
    ...
    v = json_object_insert_before(find, obj, key.c_str(), JSON_VALUE_ARRAY);
    json_value_t *remove_val = json_object_remove(find, obj);
    json_value_destroy(remove_val);
  }
  json_array_t *arr = json_value_array(v);
...
Json Json::copy() const { return *this; }
...
void Json::push_back(const std::string &val) { push_back(val.c_str()); }
...
void Json::update_arr(const std::string &val) { update_arr(val.c_str()); }
...
void Json::update_arr(const std::vector<std::string> &val) {
  json_array_t *arr = json_value_array(parent_);
  const json_value_t *v =
      json_array_insert_before(node_, arr, JSON_VALUE_ARRAY);
  json_value_t *remove_val = json_array_remove(node_, arr);
  json_value_destroy(remove_val);
