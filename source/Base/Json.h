...
template <typename T> struct is_string : std::integral_constant<bool, false> {};
...
public:
  ...
  std::string key() const { return parent_key_; }
...
public:
  ...
  void push_back(const std::string &key,
                 const std::initializer_list<std::string> &val) {
...
public:
  ...
  private:
    ...
    bool is_end() { return json_->node_ == nullptr && key_ == nullptr; }
  ...
  private:
    ...
    bool is_rend() { return json_->node_ == nullptr && key_ == nullptr; }
...
private:
  ...
  bool is_placeholder() const { return is_null() && parent_ != nullptr; }
...
protected:
  // watcher
  Json(const json_value_t *node, const json_value_t *parent, std::string &&key);
...
public:
  ...
  Object_S(std::initializer_list<pair_type> list) {
    std::for_each(list.begin(), list.end(), [this](const pair_type &pair) {
